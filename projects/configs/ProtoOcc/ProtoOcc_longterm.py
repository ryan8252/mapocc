...
num_class = 18
voxel_out_channels = 64
OccHead_feat_channel = voxel_out_channels
OccHead_output_channel = voxel_out_channels
...
model = dict(
    ...
    img_view_transformer=dict(
        type='LSSViewTransformerBEVStereo_w_context',
    ...
    dual_branch_encoder=dict(
        ...
        bev_feat_ch1 = bev_numC_Trans*2,
        bev_feat_ch2 = bev_numC_Trans*4,
        bev_feat_ch3 = bev_numC_Trans*8,
        with_cp=True,
        down_sample_for_3d_pooling=[numC_Trans*grid_size[2], bev_numC_Trans*2],
        bev_encoder_backbone=dict(
            type='CustomBEVBackbone',
            stride=[2, 2, 2],
            numC_input=bev_numC_Trans*2,
            num_channels=[bev_numC_Trans * 2, bev_numC_Trans * 4, bev_numC_Trans * 8],),
        bev_encoder_neck=dict(
            type='Custom_FPN_LSS',
            catconv_in_channels1=bev_numC_Trans * 8 + bev_numC_Trans * 4,
            catconv_in_channels2=bev_numC_Trans * 2 + voxel_out_channels * 2,
    ...
    down_sample_before_bevencoding=[numC_Trans*(len(range(*multi_adj_frame_id_cfg))+1), numC_Trans],
    # ==========================================================================
    prototype_query_decoder=dict(
        type='Prototype_Query_Decoder_nuScenes',
        with_cp = True,
        feat_channels=OccHead_feat_channel,
        out_channels=OccHead_output_channel,
        num_queries=18,
