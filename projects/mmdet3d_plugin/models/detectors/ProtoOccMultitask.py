...
class ProtoOccMultitask(BEVDet):
    ...
    def simple_test(self,
        ...
        if hasattr(self.proto_map_head, 'select_output'):
            map_logits = self.proto_map_head.select_output(
                coarse_pred, final_masks)
        else:
            map_logits = final_masks
        map_probs = self.proto_map_head.predict(map_logits).detach().cpu().numpy()
