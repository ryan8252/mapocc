...
class MAESTROProtoOccHybrid2Task(BEVDet):
    """MAESTRO 2-task front-end with PQD-style CPG and ProtoOcc OCC head."""
    ...
    def __init__(self,
                 ...
                 prototype_seed_decoder=None,
                 maestro_cpg=None,
                 maestro_map_tsfg=None,
                 maestro_occ_tsfg=None,
                 maestro_spa=None,
                 occ_feature_branch=None,
                 map_feature_branch=None,
                 occ_hfm_fusion=None,
                 map_fpn_mixer=None,
                 cnn3d_decoder=None,
                 ...
                 free_label=17,
                 **kwargs):
        super(MAESTROProtoOccHybrid2Task, self).__init__(**kwargs)
        ...
        self.dual_branch_encoder = (
            builder.build_backbone(dual_branch_encoder)
            if dual_branch_encoder is not None else None)
        self.prototype_seed_decoder = (
            build_head(prototype_seed_decoder)
            if prototype_seed_decoder is not None else None)
        ...
        self.occ_feature_branch = (
            builder.build_backbone(occ_feature_branch)
            if occ_feature_branch is not None else None)
        self.map_feature_branch = (
            builder.build_backbone(map_feature_branch)
            if map_feature_branch is not None else None)
        self.occ_hfm_fusion = (
            builder.build_backbone(occ_hfm_fusion)
            if occ_hfm_fusion is not None else None)
        self.map_fpn_mixer = (
            builder.build_backbone(map_fpn_mixer)
            if map_fpn_mixer is not None else None)
    ...
    def _build_shared_feature(self, voxel_feat):
        if self.dual_branch_encoder is not None:
            return self._split_encoder_output(
                self.dual_branch_encoder(voxel_feat))
        return self._to_maestro_layout(voxel_feat)

    def _run_prototype_seed_decoder(self, shared_feature):
        if self.prototype_seed_decoder is None:
            return None, None
        return self.prototype_seed_decoder(
            shared_feature.permute(0, 1, 4, 2, 3))
    ...
    def _validate_occ_crop_config(self):
        ...
        if (shared_x - occ_x) % 2 != 0 or (shared_y - occ_y) % 2 != 0:
            raise ValueError(
                'MAESTROProtoOccHybrid2Task only supports centered OCC crops '
                f'with even spatial margins, got shared={self.grid_size}, '
    ...
    def _run_maestro_branches(self,
                              ...
                              mask_camera=None,
                              gt_masks_bev=None):
        cpg_output = self.maestro_cpg(
            shared_feature, occ_pred=seed_occ_pred, mask_feat=seed_mask_feat)
        ...
        if self.map_feature_branch is not None:
            map_feature = self.map_feature_branch(map_feature)
        map_head_feature = map_feature
        if self.map_fpn_mixer is not None:
            map_head_feature = self.map_fpn_mixer(map_head_feature)
        map_logits = self.bev_seg_head(map_head_feature)

        if self.maestro_spa is not None:
            background_for_occ = self.maestro_spa(
                cpg_output['background'], map_head_feature, map_logits)
        else:
        ...
        if self.occ_feature_branch is not None:
            occ_feature = self.occ_feature_branch(occ_feature)
        if self.occ_hfm_fusion is not None:
            occ_feature = self.occ_hfm_fusion(occ_feature, map_head_feature)
        ...
        return occ_feature, map_head_feature, map_logits, aux_losses
    ...
    def forward_train(self,
        ...
        shared_feature = self._build_shared_feature(voxel_feat)
        seed_occ_pred, seed_mask_feat = self._run_prototype_seed_decoder(
            shared_feature)

        gt_masks_bev = self._normalize_map_targets(gt_masks_bev)
        if gt_masks_bev is None:
            raise ValueError(
                'Expected `gt_masks_bev` when training '
                'MAESTROProtoOccHybrid2Task.')

        occ_feature, map_feature, map_logits, aux_losses = (
            self._run_maestro_branches(
                shared_feature,
                seed_occ_pred=seed_occ_pred,
                seed_mask_feat=seed_mask_feat,
    ...
    def simple_test(self,
        ...
        for batch_index, occ_pred in enumerate(occ_preds):
            ...
            if gt_masks_bev is not None:
                sample['gt_masks_bev'] = gt_masks_bev[batch_index].astype(
                    np.int64)
