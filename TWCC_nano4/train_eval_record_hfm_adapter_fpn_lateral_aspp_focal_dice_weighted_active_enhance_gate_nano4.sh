...
# ProtoOcc + Map-HFM adapter + FPN lateral ASPP map neck
# + focal-Dice weighted map loss + map-active feature enhancement gate.
#
# Default regime:
...
#   sbatch TWCC_nano4/train_eval_record_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_nano4.sh
#
# This wraps quick_test_nano4.sh so the container setup and result recorder stay
# identical to the known-good Nano4 workflow.

#SBATCH -J hfm_actgate_24
...
export CONFIG="${CONFIG:-projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate.py}"
export WORK_DIR="${WORK_DIR:-${PROTOOCC_DIR}/work_dirs/ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_nano4_h200}"

# H200 full-run regime: 8 GPUs * 4 samples/GPU = 32, workers/GPU = 1.
# LR is scaled to 4e-4 for this doubled global batch. Keep workers low first
# because this map-heavy workflow is host-memory sensitive.
export GPUS="${GPUS:-8}"
