...
# ProtoOcc BEVFusion-aligned map-only classwise BEVFusion focal diagnostic.
#
# Default regime:
...
#   sbatch TWCC_nano4/train_eval_record_bevfusion_aligned_map_only_classwise_bevfusion_focal_nano4.sh
#
# 1-GPU smoke example:
#   RUN_MODE=smoke GPUS=1 SAMPLES_PER_GPU=2 WORKERS_PER_GPU=1 \
#   sbatch --export=ALL -p dev --gres=gpu:1 --cpus-per-task=8 --mem=256G \
#     --time=03:59:00 TWCC_nano4/train_eval_record_bevfusion_aligned_map_only_classwise_bevfusion_focal_nano4.sh
#
...
export CONFIG="${CONFIG:-projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_bevfusion_aligned_map_only_classwise_bevfusion_focal.py}"

case "${RUN_MODE}" in
    smoke)
        DEFAULT_EPOCHS=1
        DEFAULT_TRAIN_ANN_FILE="data/nuscenes/bevdetv2-nuscenes_infos_train_1quarter_seed0.pkl"
        DEFAULT_WORK_DIR="${PROTOOCC_DIR}/work_dirs/smoke_bevfusion_aligned_map_only_classwise_bevfusion_focal_1quarter_nano4_h200"
    ...
    full)
        ...
        DEFAULT_WORK_DIR="${PROTOOCC_DIR}/work_dirs/ProtoOcc_multi_cnn_head_map_neck_bevfusion_aligned_map_only_classwise_bevfusion_focal_nano4_h200"
...
# This follows the proven aligned map-only launcher so this ablation isolates
# BEVFusion classwise focal aggregation rather than changing optimization size.
...
# Keep checkpoints every 3 epochs and train-time eval every epoch, matching the
# aligned map-only launcher. This preserves mid-run EMA checkpoints in case the
# stronger classwise map pressure overfits or destabilizes later epochs.
