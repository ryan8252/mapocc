...
#   aligned active-gate MTL mapw8 + K=1 learned-Z residual cue.
#
# Submit from the ProtoOcc checkout on Nano4:
#   sbatch TWCC_nano4/train_eval_record_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_bevfusion_aligned_mapw8_zaware_residual_k1_nano4.sh
#
# 1-GPU smoke example:
#   RUN_MODE=smoke GPUS=1 SAMPLES_PER_GPU=2 WORKERS_PER_GPU=1 \
#   sbatch --export=ALL -p dev --gres=gpu:1 --cpus-per-task=8 --mem=256G \
#     --time=03:59:00 TWCC_nano4/train_eval_record_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_bevfusion_aligned_mapw8_zaware_residual_k1_nano4.sh
#
...
export CONFIG="${CONFIG:-projects/configs/LSS2D/ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_bevfusion_aligned_mapw8_zaware_residual_k1.py}"

case "${RUN_MODE}" in
    smoke)
        DEFAULT_EPOCHS=1
        DEFAULT_TRAIN_ANN_FILE="data/nuscenes/bevdetv2-nuscenes_infos_train_1quarter_seed0.pkl"
        DEFAULT_WORK_DIR="${PROTOOCC_DIR}/work_dirs/smoke_ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_bevfusion_aligned_mapw8_zaware_residual_k1_1quarter_nano4_h200"
    ...
    full)
        ...
        DEFAULT_WORK_DIR="${PROTOOCC_DIR}/work_dirs/ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_bevfusion_aligned_mapw8_zaware_residual_k1_nano4_h200"
