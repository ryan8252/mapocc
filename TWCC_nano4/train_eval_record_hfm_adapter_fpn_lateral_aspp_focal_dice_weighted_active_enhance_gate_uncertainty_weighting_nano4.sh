...
# Nano4 full train+eval launcher for grouped uncertainty loss weighting.

#SBATCH -J unc_actgate
#SBATCH --account=mst109130
#SBATCH -p 8gpus
#SBATCH -N 1
...
export CONFIG="${CONFIG:-projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_uncertainty_weighting.py}"
export WORK_DIR="${WORK_DIR:-${PROTOOCC_DIR}/work_dirs/ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted_active_enhance_gate_uncertainty_weighting_nano4_h200}"
...
export CHECKPOINT_INTERVAL="${CHECKPOINT_INTERVAL:-4}"
