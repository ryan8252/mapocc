...
# ProtoOcc BEVFusion-aligned dbound60 coarse-map ablation.
#
# Default mode is a smoke A/B run:
#   1 epoch, 1-quarter nuScenes train split, train -> eval -> result.md.
#
# Submit from Nano4 login node:
#   sbatch TWCC_nano4/train_eval_record_bevfusion_aligned_dbound60_coarse_map_nano4.sh
#
# Full training example:
#   RUN_MODE=full sbatch --export=ALL TWCC_nano4/train_eval_record_bevfusion_aligned_dbound60_coarse_map_nano4.sh
#
# Quick-test default:
#   8 GPUs * 2 samples/GPU = 16, workers/GPU = 1, lr = 2e-4.

#SBATCH -J db60_coarse_rec
...
CONFIG="projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_bevfusion_aligned_dbound60_coarse_map.py"
...
case "${RUN_MODE}" in
    smoke)
        ...
        DEFAULT_WORK_DIR="${PROTOOCC_DIR}/work_dirs/smoke_bevfusion_aligned_dbound60_coarse_map_1quarter_nano4_h200"
        ;;
    full)
        DEFAULT_EPOCHS=24
        DEFAULT_TRAIN_ANN_FILE=""
        DEFAULT_WORK_DIR="${PROTOOCC_DIR}/work_dirs/ProtoOcc_multi_cnn_head_map_neck_bevfusion_aligned_dbound60_coarse_map_nano4_h200"
        ;;
    *)
...
- Protocol: dbound60 coarse-map; internal map-neck 128x128, final map output 200x200
