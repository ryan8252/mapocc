#!/usr/bin/env python3
"""Sequential 1-quarter train/eval/result runner for map-head smoke configs.

Default queue:
1. aux_loss: eval + record first, reusing epoch_1_ema.pth if present.
2. focal_dice_weighted: train, eval, record.
3. coordconv_xy: train, eval, record.

The script is intentionally single-process at the config level. It never starts
the next config until the current train/eval command has exited and result.md is
written.
"""
...
JOBS: Dict[str, JobSpec] = {
    "aux_loss": JobSpec(
        name="aux_loss",
        config="projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_aux_loss.py",
        work_dir="work_dirs/smoke_ProtoOcc_multi_cnn_head_map_neck_aux_loss_1quarter_4090",
        train_port=29609,
        eval_port=29610,
        eval_first=True,
    ),
    "focal_dice_weighted": JobSpec(
        name="focal_dice_weighted",
        config="projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_focal_dice_weighted.py",
        work_dir="work_dirs/smoke_ProtoOcc_multi_cnn_head_map_neck_focal_dice_weighted_1quarter_4090",
        train_port=29611,
        eval_port=29612,
    ),
    "coordconv_xy": JobSpec(
        name="coordconv_xy",
        config="projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_coordconv_xy.py",
        work_dir="work_dirs/smoke_ProtoOcc_multi_cnn_head_map_neck_coordconv_xy_1quarter_4090",
        train_port=29613,
        eval_port=29614,
    ),
}

DEFAULT_QUEUE = ["aux_loss", "focal_dice_weighted", "coordconv_xy"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sequential train/eval/result runner for ProtoOcc map smoke configs."
    )
    parser.add_argument(
        "--jobs",
        nargs="+",
        ...
        help="Jobs to run in order. Default: aux_loss focal_dice_weighted coordconv_xy.",
    )
