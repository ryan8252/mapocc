#!/usr/bin/env python3
"""4090 smoke runner for aligned map-only classwise BEVFusion focal.

Workflow:
    train 1 epoch on nuScenes 1/4 split
    -> eval epoch_1_ema.pth with miou + map-miou
    -> write work-dir result.md

Usage:
    conda run -n mapocc python tools/codex_run_bevfusion_aligned_map_only_classwise_bevfusion_focal.py
    python tools/codex_run_bevfusion_aligned_map_only_classwise_bevfusion_focal.py --dry-run
"""
...
PROTOCOL_NOTE = (
    "BEVFusion-aligned map-only classwise focal: shared canvas +-51.2/0.4 "
    "(256), map supervision +-50/0.5 (200), focal gamma=2 alpha=-1, "
    "six per-class focal losses summed without num-class normalization"
)
REFERENCE_NOTE = (
    "A/B references: aligned map-only 4090 smoke should be compared against "
    "work_dirs/smoke_bevfusion_aligned_map_only_1quarter_4090 if available; "
    "full-run references include aligned_map_only_nano4_h200 and "
    "aligned_focal_weight128_nano4_h200, but those are not the same smoke ruler."
)
...
JOBS: Dict[str, JobSpec] = {
    "aligned_map_only_classwise_bevfusion_focal": JobSpec(
        name="aligned_map_only_classwise_bevfusion_focal",
        config=(
            "projects/configs/ProtoOcc/"
            "/home/robot/Desktop/Ryan/ProtoOcc/work_dirs/smoke_bevfusion_aligned_map_only_classwise_bevfusion_focal_1quarter_4090/result.md.py"
        ),
        work_dir=(
            "work_dirs/"
            "smoke_bevfusion_aligned_map_only_classwise_bevfusion_focal_1quarter_4090"
        ),
        train_port=29661,
        eval_port=29662,
    ),
...
DEFAULT_QUEUE = ["aligned_map_only_classwise_bevfusion_focal"]
...
def resolve_jobs(args: argparse.Namespace) -> Dict[str, JobSpec]:
    jobs = dict(JOBS)
    job = jobs["aligned_map_only_classwise_bevfusion_focal"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train/eval/record a 4090 1-quarter smoke run for aligned map-only "
            "classwise BEVFusion focal."
        )
    )
    parser.add_argument(
        "--jobs",
