#!/usr/bin/env python3
"""Run smoke tests for new map-direction configs on the combined candidate."""
...
CONFIG_ROOT = "projects/configs/ProtoOcc"
WORK_ROOT = "work_dirs"
BASE_STEM = "ProtoOcc_multi_cnn_head_map_neck_hfm_adapter_fpn_lateral_aspp_focal_dice_weighted"

JOBS: Dict[str, JobSpec] = {
    "thin_boundary": JobSpec(
        name="thin_boundary",
        config=f"{CONFIG_ROOT}/{BASE_STEM}_thin_boundary.py",
        work_dir=f"{WORK_ROOT}/smoke_{BASE_STEM}_thin_boundary_1quarter_4090",
        train_port=29619,
        eval_port=29620,
    ),
    "residual_gates": JobSpec(
        name="residual_gates",
        config=f"{CONFIG_ROOT}/{BASE_STEM}_residual_gates.py",
        work_dir=f"{WORK_ROOT}/smoke_{BASE_STEM}_residual_gates_1quarter_4090",
        train_port=29621,
        eval_port=29622,
    ),
    "thin_roi_refine": JobSpec(
        name="thin_roi_refine",
        config=f"{CONFIG_ROOT}/{BASE_STEM}_thin_roi_refine.py",
        work_dir=f"{WORK_ROOT}/smoke_{BASE_STEM}_thin_roi_refine_1quarter_4090",
        train_port=29623,
        eval_port=29624,
    ),
}

DEFAULT_QUEUE = ["thin_boundary", "residual_gates", "thin_roi_refine"]
...
def train_command(args: argparse.Namespace, job: JobSpec) -> List[str]:
    return [
        "conda",
        "run",
        "--no-capture-output",
...
def eval_command(
    args: argparse.Namespace, job: JobSpec, ckpt: Path
) -> List[str]:
    return [
        "conda",
        "run",
...
def run_job(args: argparse.Namespace, job: JobSpec) -> int:
    work_dir = ROOT / job.work_dir
    ...
    log(f"[{job.name}] config: {job.config}")
    log(f"[{job.name}] work_dir: {job.work_dir}")
    if not config_path.exists():
        log(f"[{job.name}] missing config: {job.config}")
        return 2

    train_cmd = train_command(args, job)
    ...
    eval_cmd = eval_command(args, job, ckpt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run 1quarter smoke train/eval for the new HFM-adapter + "
            "FPN-ASPP map directions."
        )
    )
    parser.add_argument(
        "--jobs",
        ...
        help="Jobs to run in order. Default: thin_boundary residual_gates thin_roi_refine.",
    )
...
def main() -> int:
    ...
    for job_name in args.jobs:
        code = run_job(args, JOBS[job_name])
        if code != 0:
            log(f"queue stopped at {job_name} with code {code}")
            return code
    log("queue finished")
    return 0
