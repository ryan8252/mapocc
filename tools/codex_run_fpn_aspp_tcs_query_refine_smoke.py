#!/usr/bin/env python3
"""Run 1-quarter smoke tests for TCS and 30-query map refinement configs."""
...
BASE = "ProtoOcc_multi_cnn_head_map_neck_fpn_lateral_aspp_focal_dice_weighted"
...
JOBS: Dict[str, JobSpec] = {
    "tcs": JobSpec(
        name="tcs",
        config=f"{CONFIG_ROOT}/{BASE}_tcs.py",
        work_dir=f"work_dirs/smoke_{BASE}_tcs_1quarter_4090",
        train_port=29631,
        eval_port=29632,
    ),
    "query_refine": JobSpec(
        name="query_refine",
        config=f"{CONFIG_ROOT}/{BASE}_query_refine.py",
        work_dir=f"work_dirs/smoke_{BASE}_query_refine_1quarter_4090",
        train_port=29633,
        eval_port=29634,
    ),
}
DEFAULT_QUEUE = ["tcs", "query_refine"]
...
def eval_command(args: argparse.Namespace, job: JobSpec, ckpt: Path) -> List[str]:
    return [
        "conda",
        "run",
        "--no-capture-output",
...
"""
    ...
    log(f"[{job.name}] wrote {rel(result_md)}")
...
def run_job(args: argparse.Namespace, job: JobSpec) -> int:
    ...
    if args.force_train or ckpt is None:
        ...
        if train_code != 0:
            write_result(
                args, job, train_status, "skipped_train_failed",
                train_cmd, None, ckpt, train_stdout, eval_stdout)
            return train_code
    else:
    ...
    if ckpt is None:
        ...
        write_result(
            args, job, train_status, "skipped_no_checkpoint",
            train_cmd, None, ckpt, train_stdout, eval_stdout)
        return 3
    ...
    write_result(
        args, job, train_status, eval_status,
        train_cmd, eval_cmd, ckpt, train_stdout, eval_stdout)
    return eval_code


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sequential TCS/query-refine 1quarter smoke runner.")
    parser.add_argument(
        "--jobs",
        nargs="+",
        choices=sorted(JOBS),
        default=DEFAULT_QUEUE,
        help="Jobs to run in order. Default: tcs query_refine.",
    )
...
def main() -> int:
    ...
    for job_name in args.jobs:
        codes.append(run_job(args, JOBS[job_name]))
    log("queue finished")
