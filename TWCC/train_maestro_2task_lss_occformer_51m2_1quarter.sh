...
# TWCC sbatch script for MAESTRO 2-task LSS + OccFormer on 1/4 nuScenes train.
# Usage:
#   sbatch train_maestro_2task_lss_occformer_51m2_1quarter.sh

#SBATCH -J maestro_2task_51m2_1q
...
CONFIG="projects/configs/MAESTRO/MAESTRO_2task_lss_occformer_51m2.py"
WORK_DIR="${PROTOOCC_DIR}/work_dirs/MAESTRO_2task_lss_occformer_51m2_1quarter_seed0"
PRETRAIN_CKPT="${PROTOOCC_DIR}/ckpts/bevdet-r50-4d-depth-cbgs_depthnet_modify.pth"
ANN_FILE="${PROTOOCC_DIR}/data/nuscenes/bevdetv2-nuscenes_infos_train_1quarter_seed0.pkl"
...
if [ ! -f "${ANN_FILE}" ]; then
    echo "[ERROR] 1/4 train annotation file not found:"
    echo "        ${ANN_FILE}"
    echo "        Copy or generate bevdetv2-nuscenes_infos_train_1quarter_seed0.pkl first."
    exit 1
fi
...
echo "[INFO] Train ann file: ${ANN_FILE}"
echo "[INFO] GPUs: ${GPUS}"
