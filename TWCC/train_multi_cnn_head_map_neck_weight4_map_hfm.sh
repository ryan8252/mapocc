...
# TWCC sbatch script for ProtoOcc + CNN map neck + map_loss_weight=4 + Map-HFM.
# Usage:
#   sbatch train_multi_cnn_head_map_neck_weight4_map_hfm.sh


#SBATCH -J map_hfm_w4                            # 任務名稱
...
CONFIG="projects/configs/ProtoOcc/ProtoOcc_multi_cnn_head_map_neck_weight4_map_hfm.py"
WORK_DIR="${PROTOOCC_DIR}/work_dirs/ProtoOcc_multi_cnn_head_map_neck_weight4_map_hfm"
PRETRAIN_CKPT="${PROTOOCC_DIR}/ckpts/bevdet-r50-4d-depth-cbgs_depthnet_modify.pth"
GPUS=4
SAMPLES_PER_GPU=4
WORKERS_PER_GPU=1

# Keep the same optimization setting as the 4-GPU ProtoOcc baseline:
# global batch = 4 GPUs * 4 samples/GPU = 16.
LR=2e-4
